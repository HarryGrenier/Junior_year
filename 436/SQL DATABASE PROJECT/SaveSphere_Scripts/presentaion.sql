...
-- ## SELECT
-- # Top ten savers
select Name, TotalSavings, TotalExpenses, (totalsavings - totalexpenses) as Total from Users ORDER by Total desc LIMIT 10;

-- # Find users in the negitve
SELECT Name, TotalSavings, TotalExpenses, (totalsavings - totalexpenses) as Total FROM Users WHERE TotalSavings - TotalExpenses < 0;

-- # Counting the number of users whose names start with H
SELECT COUNT(*) AS UsersStartingWithH FROM Users WHERE Name LIKE 'H%';

--# Show users where name starts with H
...
DELETE FROM Users WHERE Name LIKE 'H%';

-- # Remove a challange
DELETE FROM Challenges WHERE ChallengeID = 1;
...
SELECT COUNT(*) AS UsersWithSavingsBetween8000And10000 FROM Users WHERE TotalSavings > 8000 AND TotalSavings < 10000;

-- # Update all users between 8000 and 10000 to be == 10000
UPDATE Users SET TotalSavings = 10000 WHERE TotalSavings > 8000 AND TotalSavings < 10000;

-- # Checking the update results
SELECT COUNT(*) AS UsersWith10000Savings FROM Users WHERE TotalSavings = 10000;

-- # Counting users with TotalSavings less than 8000
SELECT COUNT(*) AS UsersWithLessThan8000Savings FROM Users WHERE TotalSavings < 8000;
...
SELECT name, email FROM Users WHERE UserID IN (SELECT UserID FROM Savings GROUP BY UserID HAVING AVG(Amount) > (SELECT AVG(Amount)FROM Savings));

-- # Finds the average total savings across all users
SELECT Name,TotalSavings FROM Users WHERE TotalSavings > (SELECT AVG(TotalSavings) FROM Users) ORDER BY TotalSavings DESC;

   
-- ## Outer-Join Queries
-- # Finding all users and their group memberships, including those without a group
SELECT Users.Name, Groups.GroupName FROM Users LEFT JOIN UserGroups ON Users.UserID = UserGroups.UserID LEFT JOIN Groups ON UserGroups.GroupID = Groups.GroupID;

-- ## Inner-Join Queries
-- # Finding all users and their group memberships, excluding those without a group
SELECT Users.Name, Groups.GroupName FROM Users INNER JOIN UserGroups ON Users.UserID = UserGroups.UserID 
...
SELECT c.ChallengeID, c.Name, SUM(s.Amount) AS TotalSavings FROM Challenges c JOIN UserChallenges uc ON c.ChallengeID = uc.ChallengeID JOIN Savings s ON uc.UserID = s.UserID
...
CREATE VIEW NetSavings as SELECT u.UserID, u.Name, (COALESCE(SUM(s.Amount), 0) - COALESCE(SUM(e.Amount), 0)) AS NetAmount FROM Users u
LEFT JOIN Savings s ON u.UserID = s.UserID
LEFT JOIN Expenses e ON u.UserID = e.UserID
GROUP BY u.UserID, u.Name;

-- # Showing data from the view
SELECT * FROM NetSavings;

SELECT * FROM NetSavings WHERE NetAmount > 0;
...
ALTER TABLE Users ADD CONSTRAINT chk_totalsavings CHECK (TotalSavings >= 0);

-- # Showing the CHECK constraint
SELECT * FROM information_schema.check_constraints WHERE constraint_name = 'chk_totalsavings';

-- # Dropping the CHECK constraint
ALTER TABLE Users DROP CONSTRAINT chk_totalsavings;

--# Create an index for the users email
